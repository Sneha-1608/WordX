...
<<<<<<< HEAD
  ...
  updateSegmentFromCollab: (id, targetText) => {
    set({
      segments: get().segments.map((s) =>
        s.id === id ? { ...s, targetText } : s
      ),
    });
  },

  // Collab: update status from another user's approval/rejection
  updateSegmentStatusFromCollab: (id, status) => {
    const newStatus = status as 'APPROVED' | 'REJECTED' | 'PENDING';
    const segments = get().segments.map((s) =>

...
class InferenceConfig:
    ...
    num_beams:  int = 4           # v2-fast: 4 (was 8)

    # Multi-temperature candidate generation
    mbr_beam_candidates: int   = 4     # v2-fast: 4 (was 8)
    mbr_sample_temps:    List[float] = field(
        default_factory=lambda: [0.85]
    )                                   # v2-fast: single temp (was 3)
    mbr_samples_per_temp: int  = 2     # v2-fast: 2 (was 3)

    # MBR
    mbr_utility: str = "chrf"   # v2-fast: chrF (was "comet") — 100x faster

    # Reranking
    rerank_top_k: int = 2         # v2-fast: 2 (was 4)
...
def comet_mbr_select(
    ...
    SLOW but most accurate. Used when mbr_utility="comet".
    """
    n = len(candidates)
    if n == 1:
...
def chrf_mbr_select(
    ...
    """
    Fast MBR selection using chrF as the utility function.
    No GPU needed — runs in <10ms even with 10+ candidates.
    Used when mbr_utility="chrf".
    """
    n = len(candidates)
    if n == 1:
        return candidates[0], [1.0]
    ...
    for i in range(n):
        ...
        for j in range(n):
            if i != j:
                score = chrf.corpus_score([candidates[i]], [[candidates[j]]]).score
                scores.append(score)
        utility[i] = np.mean(scores) if scores else 0.0
...
def translate(
    ...
    """
    VerbAI inference v2-fast for a single segment.

    Pipeline
    --------
    1. Constrained beam search (hard glossary enforcement)
    2. Single-temperature sampling (1 temp × 2 samples)
    3. chrF-MBR selection (instant, no GPU) OR COMET-MBR (slow, accurate)
    4. COMETkiwi reranking of top-2
    ...
    # ── Step 3: MBR selection (chrF = fast, COMET = slow) ────────────────
    if cfg.mbr_utility == "chrf":
        mbr_best, mbr_utilities = chrf_mbr_select(all_candidates)
    else:
